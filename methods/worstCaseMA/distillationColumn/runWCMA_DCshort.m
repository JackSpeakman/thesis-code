function [rk,uk,yk,objk,conk] = runWCMA_DCshort(varargin)
% run the distillation column using worst-case MA. 
...
opts = optimoptions('fminimax','Display','off','StepTolerance',1e-9,'ConstraintTolerance',1e-9,'OptimalityTolerance',1e-9);
...
for k = 2:k_max
    ...
    % run optimization
    [rOpt,~,~,flag] = fminimax(@(r)objF(r),r0,[],[],[],[],[68,82],[74,86],@(r)(deal(conF(r),[])),opts);
