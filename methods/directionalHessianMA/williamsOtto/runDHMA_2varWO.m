function [uk,yk,conk,objk] = runDHMA_2varWO(varargin)
...
fileName = 'Sbar\SbarData\Sbar01.mat'; % file name
...
n_th = size(data.th,1);
data.th = [data.th;data.th_nom];

model = @(u,i)WOplantFun([uRest(u),T],yGuess,data.th(i+1,:));
...
% fix conFun size
size_c = size(conFun(uNorm(uGuess),yGuess));
n_c = prod(size_c);
if size_c(1) ~= 1
...
for k = 2:kmax
    ...
    xObj = @(u)(objk(k-1,:)+(dobjpdu)'*(u-uk(k-1,:))'+calcDelta2D(uk(k-1,:),u,data.uu1,data.uu2,data.al,data.Sbar.obj));
    xCon = @(u)(conk(k-1,:)+(dconpdu)'*(u-uk(k-1,:))'+calcDelta2D(uk(k-1,:),u,data.uu1,data.uu2,data.al,data.Sbar.con));
