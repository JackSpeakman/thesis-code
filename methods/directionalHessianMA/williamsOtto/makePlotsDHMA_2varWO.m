% makes the plots for DHMA - 2 variable used in the thesis
...
plant = @(u)(WOplantFun([uRest(u),T]));
objFun = @(u,y)WOobjFun([uRest(u),T],y);
fminconopts = optimoptions('fmincon','Display','off');

uOptp = fmincon(@(u)objFun(u,plant(u)),uNorm(uGuess),[],[],[],[],[0,0],[1,1],...
...
%% 1. Standard MA
% run
[ukMA,ykMA,conkMA,objkMA] = runWO2_MA('K',0.5,'kmax',kmax,'T',T);
ukMAr = uRest(ukMA);
...
% run
[ukDHMA,ykDHMA,conkDHMA,objkDHMA] = runWO2_DHMA('K',0.5,'kmax',kmax,'T',T);
...
saveas(DHMAfig(1),'Plots\DHMAobj_2var.eps','epsc')
saveas(DHMAfig(1),'Plots\DHMAobj_2var.fig','fig')

saveas(DHMAfig(2),'Plots\DHMAcon_2var.eps','epsc')
saveas(DHMAfig(2),'Plots\DHMAcon_2var.fig','fig')

saveas(DHMAfig(3),'Plots\DHMAu1_2var.eps','epsc')
saveas(DHMAfig(3),'Plots\DHMAu1_2var.fig','fig')

saveas(DHMAfig(4),'Plots\DHMAu2_2var.eps','epsc')
saveas(DHMAfig(4),'Plots\DHMAu2_2var.fig','fig')
