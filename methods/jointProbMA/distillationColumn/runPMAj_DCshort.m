function [rk,uk,yk,objk,conk] = runPMAj_DCshort(varargin)
...
% print name
fprintf('\n#### Distilation Column (short) - %s - %4.2f - %2i/[%i-%i] ####\n\n','Probabilistic MA (joint)',filter,Ns,Nr(1),Nr(end));
...
fprintf('Finished PMAj Run     [%6.4fs]\n',toc)
...
%% A. Embedded Functions
    ...
    function pOut = probCalc(out,rho)
        ...
        if numel(mu) > 1
            optsFSOLVE = optimoptions('fsolve','Display','off','Algorithm','levenberg-marquardt');
            pOut = fsolve(@(x)(mvncdf([-inf,-inf],[0,0],mu-x,Sigma2)-rho),mu,optsFSOLVE);
        end
    end
